...
const { REPRODUCIBLE_POD_TEMPLATE_FILE_PATH,WORKFLOW_DESCRIPTOR_CONFIG_MAP_TEMPLATE_FILE_PATH,IRODS_PASSWORD_SECRET_TEMPLATE_FILE_PATH,GIT_PASSWORD_SECRET_TEMPLATE_FILE_PATH,PVC_TEMPLATE_FILE_PATH  } = constants
...
export async function runReproducibly(namespace: string, name: string, workflowDescriptorPath: string, irodsPassword: string, gitPassword: string, runMode: string = "REPRODUCIBLE"): Promise<k8s.KubernetesObject> {
  ...
  const maybePVCModifier = runMode === "REPRODUCIBLE" ? [] : baseModifiers.concat([{path: "metadata.name", value: pvcName}])
